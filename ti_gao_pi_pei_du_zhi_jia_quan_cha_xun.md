...
按 `heat` 字段降序排列，会导致评分最高的文档排在最后，如下
...
上面的试验没有指定加权因子，现在试下加权因子的功效，如下设置 `bq` 参数
...
在条件查询叠加加权因子的双重激励下，`神奇猪猪侠历险记` 终于超越 `钢铁侠` 排名第一了

## 多重 bq

这一次试下多重 `bq`，即 `bq=heat:99&bq=type:2`，只有 `神奇猪猪侠历险记` 同时满足这 2 个条件，能否威胁 `钢铁侠` 的榜首地位呢？
...
2. `bq` 的加权是有限的，默认情况下并不足以影响匹配度相差较大的文档之间的相对排序
3. `bq` 还可以再叠加加权因子，可调整加权程度(*加权因子小于 1 可以减轻加权程度*)
4. `bq` 可以叠加多个，对文档匹配度的提升是线性增加的
5. *`bq` 涉及的字段可以不建索引，但如果未建索引那么必须是 `docValues=true`*
